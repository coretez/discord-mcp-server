 ...
 *
 * Both the stdio entrypoint and the HTTP listener build their server through
 * here. Keeping it separate matters more than it looks: `registerTool` gates on
 * `config.mode` at registration time, so a server is only ever as capable as the
 * config it was built with. Once sessions carry their own mode, each session
 * gets its own server from this function and a read-tier caller genuinely has no
 * write tools to call rather than guarded ones.
 */
...
import { CAPABILITIES_UI_URI, SERVER_VERSION, registerMetaTools } from "./tools/meta.js";
...
export function createMcpServer(config: Config): McpServer {
  ...
  server.registerResource(
    ...
    async (uri) => ({
      contents: [{ uri: uri.href, mimeType: "text/html", text: renderCapabilitiesUi(config) }],
    }),
  );
